...
- React to messages published by the [RabbitMQ Action](./rabbit-mq-action.md) node
...
- **Dead Letter Queue:** Error output → [RabbitMQ Action](./rabbit-mq-action.md) (publish to DLQ)
- **Chained Workflows:** [RabbitMQ Action](./rabbit-mq-action.md) publish → RabbitMQ Trigger consume
...
**Solution:** Use the [RabbitMQ Action](./rabbit-mq-action.md) node with `publish` to send a test message. Use `getQueueInfo` to confirm the queue has messages and that the consumer is connected.
...
- [RabbitMQ Action](./rabbit-mq-action.md) - Publish messages and manage queues/exchanges
