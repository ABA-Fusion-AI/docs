...
Consumer groups allow multiple independent readers of the same Event Hub, each maintaining their own offset. Create additional consumer groups in the Azure portal or via the [Event Hub Admin](./event-hub-admin.md) node.
...
> Create `backfill-consumer` first using the [Event Hub Admin](./event-hub-admin.md) node.
...
- [Event Hub Send](./event-hub-send.md) - Publish events to an Azure Event Hub
- [Event Hub Admin](./event-hub-admin.md) - Create and manage Event Hubs and consumer groups
