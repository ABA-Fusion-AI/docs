---
node_id: "nextcloud-share-file"
title: "Nextcloud - Share File"
description: "Create a user, group, or public share for a Nextcloud path using the OCS Share API."
...
Create a share for a configured Nextcloud path using the OCS Share API.

The supplied implementation is named `NextcloudShareFolderNode` and has the label **Nextcloud - Share Folder**. This documentation uses **Nextcloud - Share File** to match this node directory and its workflow example. The handler sends the configured path without checking whether it identifies a file or folder.
...
The **Nextcloud - Share File** node submits a share creation request for a path in the connected Nextcloud account. Choose a user share, a group share, or a public share. The server determines whether the requested share and permissions are allowed.
...
- Share a document with a specified Nextcloud user.
- Request access for a Nextcloud group.
- Create a public share for a file when allowed by the server.
...
| `path` | string | Yes | - | Path to share in the connected account. Must contain at least one character. |
...
The supplied example connects **Manual Trigger -> Nextcloud - Share File -> Log**. It requests a user share for `Nextcloud Manual.pdf` with `shareWith` set to `1`. It omits `permissions`, so the schema default of `1` applies.

```fusion-workflow
src: example.workflow.json
title: Share a file in Nextcloud
```

Replace the example connection, path, and recipient identifier before running it. Log receives the raw response object. Inspect the OCS response to confirm share creation, and connect the error output to an error-handling step.
...
- **Nextcloud - Move File** - Move or rename a file.
...
|---------|------|---------|
| 1.0.0 | 2026-09-22 | Initial documentation for creating Nextcloud shares through the OCS Share API. |
