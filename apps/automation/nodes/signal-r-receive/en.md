...
Authentication uses the **AccessKey** from the connection string to generate a short-lived HS256 JWT (1-hour TTL) for the client connection. This is distinct from the [SignalR Action](./signal-r-action.md) node, which uses Entra ID app credentials for REST management calls.
...
- [SignalR Action](./signal-r-action.md) - Send messages and manage connections/groups via the REST API
- [Event Hub Receive](./event-hub-receive.md) - Receive high-throughput events from Azure Event Hubs
